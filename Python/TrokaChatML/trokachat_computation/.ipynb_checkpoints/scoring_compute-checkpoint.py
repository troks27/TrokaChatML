        ...
        with ProcessPoolExecutor(max_workers=num_cores) as executor:
            ...
            for file_name in self.files_list:
                if file_name.endswith('.h5'):
                    ...
                    df = pd.read_excel(f"{self.general_filepath}{self.import_folder}/{file_name}")
            ...
            # Export each .xlsx file separately
            for file_name, df_list in xlsx_files.items():
                if not isinstance(df_list, list):
                    df_list = [df_list]  # Ensure df_list is a list
            ...
            for base_name, group_dfs in h5_groups.items():
                output_filepath = os.path.join(self.output_path, f"NULLDIST_{base_name}_allpathways.h5")
